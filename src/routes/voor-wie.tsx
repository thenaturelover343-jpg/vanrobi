  ...
  head: () => ({
    meta: [
      { title: "Voor wie — VanRobi" },
      {
        name: "description",
        content:
          "Van vaste bar tot festivalterrein, dezelfde industriële betrouwbaarheid, met lokaal advies vanuit de Kempen.",
      },
    ],
  }),
  component: VoorWie,
      ...
      <section className="px-5 py-20 md:px-8">
        <div className="prose mx-auto max-w-[46rem]">
          ...
          <p>
            ...
            <Link to="/producten/$id" params={{ id: "v100" }}>V100</Link> of{" "}
            <Link to="/producten/$id" params={{ id: "v200" }}>V200</Link>). Zie de gids{" "}
            <Link to="/gids/$slug" params={{ slug: "onder-bar-bierkoeler" }}>onder-bar bierkoeler</Link>.
          </p>
          <h3>Events & festivals</h3>
          <p>
            Mobiele units zoals{" "}
            <Link to="/producten/$id" params={{ id: "v100-portable" }}>V100 portable</Link>,{" "}
            <Link to="/producten/$id" params={{ id: "v200-portable" }}>V200 portable</Link> of{" "}
            <Link to="/producten/$id" params={{ id: "goldy" }}>Goldy</Link>. Lees{" "}
